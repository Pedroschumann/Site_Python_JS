$(function() { // quando o documento estiver pronto/carregado
    ...
    function exibir_animes() {
        ...
        function listar (animes) {
            ...
            for (var i in animes) { //i vale a posição no vetor
                lin = '<tr id="linha_'+animes[i].id+'">' + // elabora linha com os dados da profissão
                '<td>' + animes[i].nome + '</td>' + 
                '<td>' + animes[i].genero + '</td>' + 
                '<td>' + animes[i].numero_de_ep + '</td>' + 
                '<td><a href=# id="excluir_' + animes[i].id + '" ' + 
                  'class="excluir_anime"><img src="img/excluir.jpeg" '+
                  'alt="Excluir anime" title="Excluir anime" height=20 width= 20></a>' + 
                '</td>' + 
                '</tr>';
    ...
    $(document).on("click", ".excluir_anime", function() {
        ...
        $.ajax({
            ...
            type: 'DELETE', 
            dataType: 'json', 
            success: animeExcluida, 
            error: erroAoExcluir
        });
        function animeExcluida (retorno) {
            if (retorno.resultado == "ok") { 
                $("#linha_" + id_anime).fadeOut(1000, function(){
                    alert("Anime removida com sucesso!");
                });
            } else {
                alert(retorno.resultado + ":" + retorno.detalhes);
            }            
